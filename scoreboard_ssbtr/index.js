    ...
    if (Object.keys(data.score[window.scoreboardNumber].team["1"].player).length == 1) {
      for (const [t, team] of [
        ...
        for (const [p, player] of [team.player["1"]].entries()) {
          if (player) {
            SetInnerHtml(
              $(`.p${t + 1}.container .player_info`),
              `
                ${t == 1 && team.losers ? `<span class='losers'>L</span>` : ""}
                ...
                ${t == 0 && team.losers ? "<span class='losers'>L</span>" : ""}
              `
            );
      ...
      for (const [t, team] of [
        ...
        nameElements.push(`<span class='team_names'>${playerNames}</span>`)
